...
int main(int argc, char **argv){
	...
	cout << "no. distance ops: " << KeyObject::n_ops << endl;
	...
	cout << "avg. no. ops: " << (double)KeyObject::n_ops/(double)NClusters << endl;
	...
	sz = mtree.size();
	cout << "size: " << sz << endl;
	assert(sz == N + NClusters*ClusterSize - 1);
	...
	assert(results.size() >= ClusterSize - 1);	
